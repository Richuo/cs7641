...
def plot_grid_value(env, V, name=""):
    if name != "":
        fig, ax = plt.subplots(1, 1, figsize=(18,9))
        ax.set_title(f"Value grid {name.replace('_', ' ')}")
        sns.heatmap(V.reshape(env.nrow, env.ncol), annot=True, fmt='.2f', linewidths=.4)
        plt.savefig(FIG_PATH+f"{name}_value_grid")
        plt.show()


def plot_heatmap_policy(policy, V, rows, columns, name=""):
    if name != "":
        ...
        fig, ax = plt.subplots(1, 1, figsize=(18,9))
        ax.set_title(f"Best action grid {name.replace('_', ' ')}")
        sns.heatmap(V.reshape(rows, columns), annot=policy_labels, fmt='', linewidths=.4)
        plt.savefig(FIG_PATH+f"{name}_action_grid")
        plt.show()
...
def value_iteration_function(env, discount_factor=0.995, max_iterations=100, plot_grid=True, name=""):
    ...
    if plot_grid:
        plot_grid_value(env, V, name)
        plot_heatmap_policy(optimal_policy_actions, V, env.nrow, env.ncol, name)
...
def policy_iteration_function(env, discount_factor=0.995, max_iterations_hl=10, max_iterations_ll=100, plot_grid=True, name=""):
    ...
    if plot_grid:
        plot_grid_value(env, optimal_V, name)
        plot_heatmap_policy(optimal_policy_actions, optimal_V, env.nrow, env.ncol, name)
...
def epsilon_greedy_policy(env, Q, epsilon):
...
def plot_average_score_parameters(average_scores, liste, episode_max_score, name=""):
    if name != "":
...
def q_learning_function(env, num_episodes=10000, discount_factor=0.995, initial_Q=None,
                        learning_rate_decay=0.999, epsilon_decay=0.999, plot_grid=True, name="", do_plot=True):
    ...
    # Plotting
    plot_average_score_parameters(average_scores, list_epsilon_lr, episode_max_score, name)
    if plot_grid:
        plot_grid_value(env, optimal_V, name)
        plot_heatmap_policy(optimal_policy_actions, optimal_V, env.nrow, env.ncol, name)
    ...
    stats = {}
    stats['max_iteration'] = episode_max_score if episode_max_score > offset else episode
    stats['max_reward'] = max_score
    stats['running_time'] = running_time_max_score if (running_time_max_score != 0 and episode_max_score > offset) else running_time
