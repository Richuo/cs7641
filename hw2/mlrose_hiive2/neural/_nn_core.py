...
from mlrose_hiive2.algorithms.decay import GeomDecay
from mlrose_hiive2.algorithms.rhc import random_hill_climb
from mlrose_hiive2.algorithms.sa import simulated_annealing
from mlrose_hiive2.algorithms.ga import  genetic_alg

from mlrose_hiive2.neural._nn_base import _NNBase
from mlrose_hiive2.neural.activation import (identity, relu, sigmoid, tanh)
from mlrose_hiive2.neural.utils.weights import gradient_descent_original, unflatten_weights
...
class _NNCore(_NNBase):
    ...
    def fit(self, X, y=None, x_test=None, y_test=None, init_weights=None):
        ...
        if self.algorithm == 'random_hill_climb':
            fitness_curve, fitted_weights, loss, state, times, iterations = self.__run_with_rhc(init_weights, num_nodes, problem)

        elif self.algorithm == 'simulated_annealing':
            fitness_curve, fitted_weights, loss, state, times, iterations = self._run_with_sa(init_weights, num_nodes, problem)
        elif self.algorithm == 'genetic_alg':
            fitness_curve, fitted_weights, loss, state, times, iterations = self._run_with_ga(problem)
        else:  # Gradient descent case
            fitness_curve, fitted_weights, loss, state, times, iterations = self._run_with_gd(init_weights, num_nodes, problem)
        ...
        for s in self.state : 
            ...
            train_loss = log_loss(y, y_train_pred)
            train_accuracy = accuracy_score(y, y_train_pred)
            train_f1 = f1_score(y, y_train_pred)

            self.train_stats.append([train_loss, train_accuracy, train_f1]) 
            ...
            self.test_stats.append([test_loss, test_accuracy, test_f1])
    ...
    def _run_with_gd(self, init_weights, num_nodes, problem):
        ...
        fitted_weights, loss, fitness_curve, state, times, iterations = gradient_descent_original(
            problem,
        ...
        return ([] if fitness_curve is None else fitness_curve), fitted_weights, loss, state, times, iterations
    ...
    def _run_with_ga(self, problem):
        ...
        if self.curve:
            ...
            fitted_weights, loss, _, state, times, iterations = genetic_alg(
                problem,
                ...
                max_iters=self.max_iters,
                curve=self.curve)
        return fitness_curve, fitted_weights, loss, state, times, iterations
    ...
    def _run_with_sa(self, init_weights, num_nodes, problem):
        ...
        if self.curve:
            ...
            fitted_weights, loss, _, state, times, iterations = simulated_annealing(
                problem,
                schedule=self.schedule,
                max_attempts=self.max_attempts if self.early_stopping else
                self.max_iters,
    ...
    def __run_with_rhc(self, init_weights, num_nodes, problem):
        ...
        for _ in range(self.restarts + 1):
            ...
            if self.curve:
                current_weights, current_loss, fitness_curve, state, times, iterations = \
                ...
                current_weights, current_loss, _, state, times, iterations = random_hill_climb(
                    problem,
